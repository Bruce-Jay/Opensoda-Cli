...
(async () => {
    ...
    if (argv.r) {
        ...
        if (argv.m) {
            ...
            const metricData: Object = await getMetric(argv.r, argv.m)
            const innerMetricData: TimeIndexedData = Object.values(metricData)[0]
            ...
            if (argv.t) {
                ...
                const value: any = innerMetricData[time];
                if (value) {
            ...
            if (argv.t) {
                ...
                // 不能在 for 循环里面用否则会乱
                const downloadUrl: string = await downloadAllMetrics(data, time);
